...
for(const g of generators){const b=document.createElement('button');b.dataset.id=g.id;b.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${g.icon}"/></svg><span>${g.name}</span>`;b.onclick=()=>{active=g;renderControls();schedule();if(narrowPanels.matches){setPanel("generators",false);setPanel("parameters",true);}};$('generators').append(b);}
function schedule(){
 if(!engine)return;
 renderVersion++;
...
function setPanel(panel,open){
 ...
 $(panel==='generators'?'collapse':'toggle-parameters').setAttribute('aria-expanded',open);
 if(open&&narrowPanels.matches)setPanel(panel==='generators'?'parameters':'generators',false);
}
$('collapse').onclick=()=>setPanel('generators',document.body.classList.contains('generators-hidden'));
$('toggle-parameters').onclick=()=>setPanel('parameters',document.body.classList.contains('parameters-hidden'));
function adaptPanels(){setPanel('generators',!narrowPanels.matches);setPanel('parameters',!narrowPanels.matches);}
narrowPanels.addEventListener('change',adaptPanels);adaptPanels();
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&narrowPanels.matches){const focusPanel=document.activeElement?.closest('aside');setPanel('generators',false);setPanel('parameters',false);if(focusPanel)$(focusPanel.id==='sidebar'?'collapse':'toggle-parameters').focus();}});
